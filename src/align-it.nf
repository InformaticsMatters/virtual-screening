...
include { concatenate_files as concat_sdf } from './nf-processes/file/concatenate_files.nf'
include { concatenate_files as concat_scores } from './nf-processes/file/concatenate_files.nf'
...
workflow align_it {
    ...
    concat_sdf(search_phar.out[0].collect(), params.output_sdf, 'aligned_*.sdf')
    concat_scores(search_phar.out[1].collect(), params.output_scores, 'aligned_*.tab')

    emit:
    sdf = concat_sdf.out
    scores = concat_scores.out
}

workflow {
    align_it(file(params.query), file(params.input))
}
