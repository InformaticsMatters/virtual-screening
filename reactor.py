...
def execute(reactions, reactants, output, read_header=False, write_header=False, interval=None):
    ...
    with open(output, 'wt') as writer:
        ...
        for reacts in product(*mols_arr):
            ...
            if interval and num_products % interval == 0:
                DmLog.emit_event("Processed {} records".format(num_products))
...
def main():
    ...
    for i, c in enumerate(reactant_counts):
        ...
        if i:
            msg += 'x'
        msg += str(c)
    DmLog.emit_event(msg, 'Products:', str(product_count), 'Combinations:', q, 'Time (s):', end - start)
